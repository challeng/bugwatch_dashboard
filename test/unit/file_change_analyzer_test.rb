...
class FileChangeAnalyzerTest < ActiveSupport::TestCase
  ...
  attr_reader :commit, :sut, :email_list, :mailer

  def setup
    @commit = Bugwatch::Commit.new(stub)
    @sut = FileChangeAnalyzer
  ...
  test "#call does not send email to groups with subscribed files not modified" do
    AppConfig.stubs(:file_changes).returns({"group1" => {"files" => %w(file.rb), "emails" => email_list}})
  ...
  test "#call matches file patterns" do
    AppConfig.stubs(:file_changes).returns({"group1" => {"files" => %w(app/*.rb), "emails" => email_list}})
